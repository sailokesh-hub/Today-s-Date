...
app.get("/todos/", async (request, response) => {
  ...
  switch (true) {
    ...
    //scenario 1
    /**-------------has only status ------------ */
    // Scenario 1: Has only status property
    case hasStatusProperty(request.query):
      if (status === "TO DO") {
        getTodosQuery = `SELECT * FROM todo WHERE status = 'TO DO';`;
        data = await database.all(getTodosQuery);
        response.send(data.map((eachItem) => outPutResult(eachItem)));
...
app.put("/todos/:todoId/", async (request, response) => {
  ...
  const {
    ...
    dueDate = previousTodo.due_date, // Corrected: changed dueDate to due_date
  } = request.body;
